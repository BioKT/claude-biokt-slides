...
\setbeamertemplate{frame footer}{%
  \raisebox{1pt}{\includegraphics[height=0.38cm]{claude_symbol.png}}%
}
...
\date{\today}
\titlegraphic{\includegraphics[height=1.2cm]{upvehu_logo_crop.pdf}\hfill\includegraphics[height=0.8cm]{claude_logo_rsvg.pdf}}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}[label=\textbullet]\setlength\itemsep{3pt}
        ...
        \item \texttt{CLAUDE.md}, skills, memory, sessions
...
\begin{frame}{Session Management}
  ...
  In the BioKT setup, the PreCompact hook runs \texttt{/ctxt save} automatically before compaction, so session state is always checkpointed.}
\end{frame}
...
"hooks": {
  ...
  The PreCompact hook shown here is one we use in the lab: when the context window fills, Claude automatically runs \texttt{/ctxt save} to checkpoint the session state before compaction.\par
...
\begin{frame}{BioKT Setup at a Glance}
  ...
  \begin{tikzpicture}[node distance=0.55cm and 0.9cm]
    ...
    \node[tbox, above=0.9cm of cc]                   (sk)  {25 Skills\\{\tiny BIOKT library}};
  ...
  \end{tikzpicture}
  \end{center}
  \note{This diagram shows the full configuration. Claude Code runs on five interactive machines and is connected to four configuration layers: CLAUDE.md (global instructions), 25 skills (the BIOKT library), project memory (16 active research projects), and settings.json.\par
...
\begin{frame}{Compute Infrastructure}
  ...
  \vspace{0.5em}
  \begin{itemize}
    \item \textbf{\texttt{/gh-propagate}} syncs \texttt{{\raise.17ex\hbox{$\scriptstyle\sim$}}/.claude} to all \checkmark\ machines via git
...
\begin{frame}{Skills Library: BioKT Collection}
  ...
  \begin{tabular}{@{}lll@{}}
    ...
    \midrule
    \multirow{3}{*}{Infrastructure}    & \texttt{/jobs}               & Track simulation jobs in JOBS.md \\
                                       & \texttt{/sim-paths}          & SSH pre-flight file check \\
                                       & \texttt{/bash-parallelize}   & Parallel subagents for independent loops \\
    \midrule
    \multirow{4}{*}{Knowledge mgmt}   & \texttt{/overview}           & Aggregate TODO/CONTEXT across machines \\
                                       & \texttt{/ctxt}               & Save/restore session context \\
  ...
  \end{center}
  \note{This table gives an overview of all 13 BioKT skills.\par
...
\begin{frame}{Knowledge \& Session Management}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{\texttt{/overview}}
        \small Aggregates \texttt{CONTEXT.md} and \texttt{TODO.md} from all interactive machines via SSH. Produces a prioritised action plan for the day or week. Run from the local Mac.
      \end{block}
      \vspace{0.5em}
      \begin{block}{\texttt{/ctxt save} / \texttt{load}}
        \small Checkpoints the current session state to \texttt{CONTEXT.md}. Run before \texttt{/clear} to avoid losing context. Restored automatically at next session start if the file is present in the project directory.
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
  ...
  \texttt{/overview} is the morning routine. Run it from the local Mac; it SSHes into all interactive machines, reads their CONTEXT.md and TODO.md files, and produces a single prioritised action plan. It answers ``what should I work on today?'' across the whole infrastructure.\par
  \texttt{/ctxt save} and \texttt{load} address the ephemeral nature of Claude sessions. \texttt{/ctxt save} writes a session summary to CONTEXT.md. Running \texttt{/ctxt load} at the next session start restores that context. The PreCompact hook triggers \texttt{/ctxt save} automatically when the context window fills.\par
...
\begin{frame}{A Day in the Lab}
  ...
  \begin{itemize}\setlength\itemsep{2pt}
    ...
    \item \textbf{End of session:} \texttt{/bitacora} captures decisions; \texttt{/ctxt save}; then \texttt{/clear}
  \end{itemize}
  ...
  End of session: \texttt{/bitacora} appends an entry to LABNOTEBOOK.md capturing what was done. \texttt{/ctxt save} checkpoints the context. \texttt{/clear} resets the window ready for the next task.\par
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}[label=\textbullet]\setlength\itemsep{4pt}
        ...
        \item End sessions with \texttt{/bitacora} and \texttt{/ctxt save}
